...
class Aead(encoding.Encodable, StringFixer):
    ...
    """
    KEY_SIZE: Incomplete
    NONCE_SIZE: Incomplete
    MACBYTES: Incomplete
    MESSAGEBYTES_MAX: Incomplete
    def __init__(self, key: bytes, encoder: encoding.Encoder = ...) -> None: ...
    def __bytes__(self) -> bytes: ...
    def encrypt(self, plaintext: bytes, aad: bytes = b'', nonce: bytes | None = None, encoder: encoding.Encoder = ...) -> EncryptedMessage:
        """
        Encrypts the plaintext message using the given `nonce` (or generates
        one randomly if omitted) and returns the ciphertext encoded with the
        encoder.
        ...
        :rtype: [:class:`nacl.utils.EncryptedMessage`]
        """
    def decrypt(self, ciphertext: bytes, aad: bytes = b'', nonce: bytes | None = None, encoder: encoding.Encoder = ...) -> bytes:
        """
        Decrypts the ciphertext using the `nonce` (explicitly, when passed as a
        parameter or implicitly, when omitted, as part of the ciphertext) and
        returns the plaintext message.
